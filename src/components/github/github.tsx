...
export default function GithubActivity() {
    ...
    useEffect(() => {
        fetchActivity().then(a => {setActivity(a); setError(false)}).catch(() => setError(true))
    }, [])
...
async function fetchActivity() : Promise<Activity[]>{
    return (async () => {
        const activityReq = await fetch(`https://api.github.com/users/BlueTree242/events`, //fetch them
...
export const events: ReadonlyArray<string> =
    ['PushEvent', 'PullRequestEvent', 'PullRequestReviewEvent', 'IssuesEvent', 'CreateEvent', 'IssueCommentEvent'] as const //supported events from github
...
export const titles: {[k: string]: ActivityHtml} = {
    ...
    'PullRequestEvent': activity =>
        ['opened', 'edited', 'closed', 'reopened', 'assigned', 'unassigned', 'created'].includes(activity.payload.action) ? {
            ...
            suffix: 'in',
            body: <span><a href={pullRequestUrl(activity)}>#{activity.payload.pull_request.number}:</a> {activity.payload.pull_request.title}</span>
        } : null,
    ...
    'CreateEvent': activity => ({ // for example creating new branch
...
function commitUrl(activity: Activity, commit: Commit) { //gets url for a commit
    return `https://github.com/${activity.repo.name}/commit/${commit.sha}`
}

function pullRequestUrl(activity: Activity) { //gets url for a pull request
    return `https://github.com/${activity.repo.name}/pull/${activity.payload.pull_request.number}`
}

function issueUrl(activity: Activity) { //gets url for an issue
